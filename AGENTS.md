...
For a no-mistakes ship, the same worker proceeds directly from its implementation commit into validation, using the harness invocation owned by `harness-adapters`, without waiting for a firstmate steer.
