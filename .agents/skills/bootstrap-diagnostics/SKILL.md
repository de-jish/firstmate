...
- `DECISION_HOLD: <identity> <defect> <remediation>` - a captain decision that is neither actively held nor durably resolved.
  ...
  Each line carries its own remediation because the defects differ: attest an answer the captain already gave with the printed `bin/fm-decision-hold.sh repair`, re-activate a decision that was released without being closed and then close it with the captain's word, or restore an identity that no longer carries the provenance `repair` needs by reopening that same identity, re-holding it with `tasks-axi hold --kind captain`, and closing the same key through this script with `resolve` when it still blocks routed work or `answer` when it does not.
  The third one never becomes a new decision key: the origin's recorded inventory still names the old key, so a decision raised under a fresh key leaves the old one unattestable and its origin unable to pass the completion gate.
