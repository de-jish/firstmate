...
- `DECISION_HOLD: <identity> <defect> <remediation>` - a captain decision that is neither actively held nor durably resolved.
  ...
  Each line carries its own remediation because the defects differ: attest an answer the captain already gave with the printed `bin/fm-decision-hold.sh repair`, re-activate a decision that was released without being closed and then close it with the captain's word, or restore an identity that no longer carries the provenance `repair` needs by reopening that same identity, re-holding it with `tasks-axi hold --kind captain`, and answering the same key through this script.
