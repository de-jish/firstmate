...
6. The normal recap branch is session-history-only, apart from the step 0 helm check.
   ...
   The only other exceptions to that list are the captain-facing question surface step 8 presents its decisions through, the mandatory `harness-adapters` load that selects it, and the `state/.afk` presence read that selection needs; every other item in the list stays banned.
