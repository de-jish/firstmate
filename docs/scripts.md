...
| `fm-decision-hold.sh`    | Create, verify, audit, complete, close, and repair durable captain-held decisions |
...
| `fm-task-mode.sh`        | Change a live ship task's mode, record it, and send the canonical worker steer        |
| `fm-merge-local.sh`      | Guard a `local-only` default-branch landing and optionally push it directly           |
