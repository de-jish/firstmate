...
Both restoring steps are `tasks-axi` commands rather than `bin/fm-decision-hold.sh hold`, because there is a reachable shape in which `hold` refuses and these two do not.
`command_hold` refuses unless the origin is owned by the active home, and `origin_exists_here` passes on ANY of three signals: `state/<origin-id>.meta` exists, `data/<origin-id>/report.md` exists, or the origin task is still in `data/backlog.md`.
Teardown clears only the first of the three - `bin/fm-teardown.sh` removes `state/<origin-id>.meta` and `state/<origin-id>.turn-ended` and nothing under `data/<origin-id>/` - and ordinary retention later archives the closed origin task out of `data/backlog.md`.
The surviving report is deliberate rather than an oversight: `AGENTS.md` records `<id>/report.md` as the scout task deliverable that survives teardown, and a non-forced scout teardown refuses outright when it is missing, so a scout origin that wrote one keeps `bin/fm-decision-hold.sh hold` working for as long as that file is on disk.
All three signals are gone only for an origin that has no `data/<origin-id>/report.md` at all, once its metadata is deleted and retention has archived its backlog row: an origin of a kind that writes no scout report, a scout discarded with `--force` before it wrote one, since that is the only path teardown does not gate on the report, or any origin whose report was removed afterwards by hand.
For an identity in that state `bin/fm-decision-hold.sh hold` exits 1 with `origin <origin-id> is not owned by the active home <FM_HOME>` and restores nothing, while the decision identity itself outlives all three signals and stays in the audit's scope through the creation body `hold` wrote on it, so the finding keeps printing with no followable remedy.
...
The two commands below act on the decision identity itself, so one recovery covers an unowned origin and a live one alike, and the reproduce-catch-revert transcripts below carry both that refusal and this recovery run against an unowned origin.
...
It is the same command the lost-provenance line prints as its own second step, and it is the only command observed to restore both `held` and `hold_kind` on an identity whose origin this home no longer owns.
...
None of those three reads the origin's metadata or report, so they close an identity whose origin is no longer owned here exactly as they close one whose origin is.
...
Unowned-origin open-state recovery, corrected teardown-deletion premise, and fourth open shape verification date: 2026-08-21.
...
A ninth walks the recorded open-state recovery for an identity whose origin this home no longer owns by any of the three signals `origin_exists_here` reads, which is the case the recovery was previously unfollowable for.
It builds that shape deliberately rather than by reproducing teardown, because teardown alone does not reach it: it removes the origin's metadata, removes the report an origin that writes no scout deliverable would never have had, closes the origin task, drives it out of `data/backlog.md` with real retention closes, and asserts all three signals are gone before the walk begins.
It first pins the documented limit by proving `bin/fm-decision-hold.sh hold` refuses there and leaves the record byte-identical, then runs the two `tasks-axi` steps in the order the section records, on the `queued held=no` shape and on the `in_flight` shape, and proves in each case that the audit falls silent and that this ledger can then close the identity with the captain's word.
...
The recovery above is written in `tasks-axi` because an earlier version of it prescribed `bin/fm-decision-hold.sh hold <origin-id> <decision-key> ...`, which refuses once none of the three signals `origin_exists_here` reads survives.
That earlier version also mis-stated why: it claimed teardown deletes `data/<origin-id>/` along with the metadata, and teardown does no such thing.
The correction is recorded rather than quietly applied, because the wrong premise made the refusal sound far more common than it is.
Teardown removes `state/<origin-id>.meta` and `state/<origin-id>.turn-ended` only, retention later archives the closed origin task, and `data/<origin-id>/report.md` survives by contract, so a scout origin that wrote its report keeps `hold` working after teardown indefinitely - which was confirmed by running it on exactly that home:

```text
$ ls data/samp/report.md ; tasks-axi show samp     # meta deleted, backlog row archived, report intact
data/samp/report.md
...
$ bin/fm-decision-hold.sh hold samp route --title ... --reason ...   # accepted, because the report still owns the origin
samp-decision-route
[exit 0]
```

The blocks that follow were therefore captured on a home whose origin has none of the three: held and completed through this ledger, then its metadata deleted, its report removed, and its task closed and driven out of `data/backlog.md` by twelve real retention closes.
That is the shape an origin which never wrote a scout report reaches on its own once teardown and retention have run, and the fixture reaches it by removing the report by hand rather than by pretending teardown did.
The refusal and the recovery were both run there, and the recovery was verified on all four open shapes the audit's fall-through fires on - `state=queued held=no`, `state=queued held=yes` with a non-captain `hold_kind`, `state=in_flight held=yes hold_kind=captain`, and `state=in_flight held=no` - with the unrouted and routed closes both walked, and each of the four is captured below.
...
The `in_flight` shape on the same unowned origin, with a routed task blocked behind the decision throughout.
The refusal comes earlier here than it does for an origin still owned here: `origin_exists_here` is checked before the queued test, so on an unowned origin the command changes nothing at all, where on an owned one it applies the captain hold and only then refuses.
...
The `state=queued held=yes` shape with a non-captain `hold_kind`, where the hold was released and then re-held for something that is not the captain, so the identity blocks work without recording a captain question.
It needs the same single restoring step as the released shape, and it is captured separately because it is the one open shape whose record already reads `held: yes`.
...
$ tasks-axi show samp-decision-route --full | grep -E "state:|held:|hold_kind:"
  ...
  hold_kind: external
$ bin/fm-decision-hold.sh audit
samp-decision-route is open in a state fm-decision-hold cannot close (state=queued held=yes hold_kind=external), so no captain answer can be recorded on it while it stays there; the recovery for each open state is in docs/decision-hold-lifecycle.md under "Recovering an open decision hold"
[exit 0]
...
not ok - the recorded recovery did not restore the captain hold on an unowned-origin identity

# the restoring steps as this section records them now
$ bash tests/fm-decision-hold-lifecycle.test.sh   # driven to the one case
ok - the recorded open-state recovery is followable when the origin is no longer owned here and ends the finding
```
...
The decision-hold suite's block below was extended rather than rewritten on the re-capture that added `the recorded open-state recovery is followable when the origin is no longer owned here and ends the finding`: every line that was already recorded is still emitted verbatim by the run that produced it, and the new one sits in its invocation position, so the block reads 27 cases where it read 26.
