# Firstmate operational-home layout

This file is the reference inventory of a firstmate home's private runtime
state: what each directory and record file is, which script owns it, and which
ones must never be hand-edited.

It was moved out of `AGENTS.md` because it is *lookup* material.
A session needs it when debugging a specific record, recovering a task, or writing a script that touches state - not on every turn.
`AGENTS.md` section 2 keeps the always-loaded half: the four top-level directories, the `FM_HOME` contract, and the rule that a status line is a wake event rather than current-state truth.

`docs/configuration.md` remains the single owner of the top-level operational-home layout and the configuration schemas.
Each producing script's header and `--help` remain the single owner of its own record's exact fields and mutation mechanics.
This file indexes them; it does not restate them.
...
## Notes carried from AGENTS.md section 2
