...
test_faster_paths_use_configured_authority_without_stacked_review() {
  ...
  brief="$home/data/$id/brief.md"
  assert_grep "The configured merge authority approves the ready branch, then firstmate lands it through the guarded fast-forward path and may explicitly push the default branch without a PR." "$brief" \
    "local-only brief lost configured merge authority and guarded landing"
  assert_grep "merge its default branch into \`fm/$id\` additively, never rebase or force" "$brief" \
    "local-only brief did not carry the additive recovery contract"
