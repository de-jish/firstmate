...
family_for_basename() {
  case "$1" in
    ...
    fm-teardown-endpoint-safety.test.sh)
      printf '%s\n' backend-dispatch
      ;;
    fm-local-delivery.test.sh|fm-pr-check-security.test.sh|fm-pr-merge.test.sh|fm-review-diff.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-timeout-lib.sh)
      ...
      printf '%s\n' secondmate
      ;;
    bin/fm-pr-*|bin/fm-merge-local.sh|bin/fm-task-mode.sh|bin/fm-teardown.sh|bin/fm-review-diff.sh|\
    ...
    bin/fm-brief.sh|bin/fm-delivery-mode-lib.sh|bin/fm-ensure-agents-md.sh|bin/fm-crew-state.sh|\
